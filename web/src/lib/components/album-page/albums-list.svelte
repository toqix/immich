<script lang="ts">
  ...
  const groupOptions: AlbumGroupOption = {
    ...
    [AlbumGroupBy.Owner]: (order, albums): AlbumGroup[] => {
      ...
      const groupedByOwnerIds = groupBy(
        albums,
        (album) => album.albumUsers.find(({ role }) => role === AlbumUserRole.Owner)!.user.id,
      );
      ...
      const sortedByOwnerNames = Object.entries(groupedByOwnerIds).sort(([ownerIdA, albumsA], [ownerIdB, albumsB]) => {
        ...
        if (ownerIdA === currentUserId) {
          ...
          const ownerA = albumsA[0].albumUsers.find(({ role }) => role === AlbumUserRole.Owner)!.user;
          const ownerB = albumsB[0].albumUsers.find(({ role }) => role === AlbumUserRole.Owner)!.user;
      ...
      return sortedByOwnerNames.map(([ownerId, albums]) => ({
        ...
        name:
          ownerId === currentUserId
            ? $t('my_albums')
            : albums[0].albumUsers.find(({ role }) => role === AlbumUserRole.Owner)!.user.name,
        albums,
      }));
    },
  };
  ...
  let showFullContextMenu = $derived(
    allowEdit &&
      selectedAlbum &&
      selectedAlbum.albumUsers.find(({ user: { id } }) => id === authManager.user.id)?.role === AlbumUserRole.Owner,
  );
